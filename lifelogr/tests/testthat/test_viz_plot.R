...
test_that("plot_sleep produces the right error messages", {
  expect_error(plot_sleep(EX), 
               '"plot_type" must be one of "by_weekday", "by_start_end_time", "by_datetime", "by_restless_prop", "by_restless_min", or "by_quality"')
  expect_error(plot_sleep(EX, "weekday"), 
               '"plot_type" must be one of "by_weekday", "by_start_end_time", "by_datetime", "by_restless_prop", "by_restless_min", or "by_quality"')
})

test_that("plot_daily produces the right error messages", {
  expect_error(plot_daily(EX), 
               '"measure_var" must be one of "steps", "floors", "distance", "calories", "mins_very", "rest_hr"')
  expect_error(plot_daily(EX, "step"), 
               '"measure_var" must be one of "steps", "floors", "distance", "calories", "mins_very", "rest_hr"')
})

test_that("plot_intraday produces the right error messages", {
  expect_error(plot_intraday(EX), 
               '"measure_var" must be one of "steps", "floors", "distance", "caloriesBurned", "activeMin", "bpm", "weight"')
